import { NODE_TYPES, ICONS, PREFIXES } from '../constants.js';
 ...
 * @returns {Result<string>} - 성공 시 포맷팅된 문자열(Ok<string>)을, 실패 시 에러(Err<Error>)를 반환합니다.
 */
export function formatTree(tree) {
  try {
     ...
     */
    function formatNodesRecursive(nodes, prefix) {
      for (let i = 0; i < nodes.length; i++) {
        const node = nodes[i];
        const isLast = i === nodes.length - 1;

        const connector = isLast ? PREFIXES.END : PREFIXES.BRANCH;
        const icon = node.type === NODE_TYPES.DIRECTORY ? ICONS.DIRECTORY : ICONS.FILE;

        lines.push(`${prefix}${connector}${icon} ${node.name}`);

        // 현재 노드가 디렉토리이고 자식이 있다면, 다음 레벨로 재귀 호출
        if (node.type === NODE_TYPES.DIRECTORY && node.children && node.children.length > 0) {
    ...
    const errorMessage = `An unexpected error occurred during formatting. Original error: ${error.message}`;
